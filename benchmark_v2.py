...
def main():
    ...
    try:
        src, mask, tgt = read_images(
            config.SRC_PATH, config.MASK_PATH, config.TGT_PATH
        )
    ...
    # 2. Initialize the OpenMP Grid Processor and swap the core for Multigrid
    print("Initializing OpenMP Multigrid v2 solver...")
    proc = GridProcessor(
        gradient="src",
        backend="openmp",
        n_cpu=config.N_CPU,
    ...
    # Swap the backend core for the new Multigrid solver
    proc.core = core_openmp.MultigridSolver(config.GRID_X, config.GRID_Y, config.N_CPU)
    ...
    # 5. Save the result
    write_image("result_v2.jpg", result)
    print("Saved result to result_v2.jpg")
